...
window_size = 5  # choose an odd number, the larger it is the smoother the result
...
for filename in os.listdir(input_dir):
    if filename.endswith(".mp4"):
        ...
        speed_smooth = abs(savgol_filter(speeds, window_size, polynomial_order))


        # Create a threshold to mark when the person performs a point
        threshold = np.percentile(speed_smooth, 80)
