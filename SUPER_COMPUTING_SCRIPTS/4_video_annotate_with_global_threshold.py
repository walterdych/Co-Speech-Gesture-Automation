...
def generate_annotations(data):  # Modified function with global thresholding
    global_threshold = np.mean(list(map(float, data['speed_smooth'])))  # Global threshold based on mean speed
     # Generate annotations based on speed_smooth data.
    ...
    for i in range(1, len(speed_smooth)):
        if speed_smooth[i] > global_threshold and speed_smooth[i-1] <= THRESHOLD:
            if last_stroke_type is None or last_stroke_type == 'Steady' or last_stroke_type == 'R':
                ...
                last_stroke_type = None
        elif speed_smooth[i] <= global_threshold and speed_smooth[i-1] > THRESHOLD:
            if last_stroke_type == 'Approach':
                ...
                last_stroke_type = None
                start_index = None
        elif last_stroke_type == 'Steady' and speed_smooth[i] > global_threshold and speed_smooth[i-1] <= THRESHOLD:
